    ...
    stages {
       ...
       stage('Trivy Scan') {
		...
		stage('Trigger CD Pipeline') {
			steps {
				script {
                    sh "curl -v -k --user clouduser:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://ec2-16-171-39-132.eu-north-1.compute.amazonaws.com:8080/job/gitops-testing-CD/buildWithParameters?token=gitops-token'"
				}
			}
		}
    }
